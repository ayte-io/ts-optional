...
describe('Optional', () => {
    ...
    describe('.require()', () => {
        for (let key in PRESENT) {
            it(`accepts ${key}`, () => {
                const callee = jest.fn(() => Optional.require(PRESENT[key]));
                expect(callee()).toBe(PRESENT[key]);
                expect(callee).toHaveBeenCalledTimes(1);
            });
        }

        for (let key in ABSENT){
            it(`throws on ${key}`, () => {
                expect(jest.fn(() => Optional.require(ABSENT[key]))).toThrow();
            });
        }

        it('allows to provide custom error', () => {
            const error = new Error();
            expect(() => Optional.require(null, () => error)).toThrowError(error);
        });
    });
